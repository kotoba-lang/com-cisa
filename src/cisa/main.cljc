(ns cisa.main
  "Kotodama WASM entrypoint for the Cisa clean-room actor (L4) — Clojure port.
  ...
  facts (`cisa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"cisa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cisa-compat" :tier tier :entities entities} 200])
